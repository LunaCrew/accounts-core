import type { Translation } from '../i18n-types'

const pt_br = {
  //* geral *//
  hi_name: 'Olá {name}!',

  //* emails geral *//
  if_you_did_not_request_this_email_ignore_it: 'Se você não solicitou este e-mail, por favor, ignore-o.',
  if_you_have_any_questions: 'Se tiver qualquer dúvida,',
  contact_us: 'fale conosco.',

  //* email código de verificação *//
  this_is_your_verification_code: 'Este é o seu código de verificação, insira-o no app.',
  hi_name_here_is_your_verification_code: 'Olá {name}, aqui está o seu código de verificação.',
  do_not_share_this_code_with_anyone: 'Não compartilhe este código com ninguém.',

  //* texto alternativo *//
  logo_alt_text: 'Luna Logo',
} satisfies Translation

export default pt_br
